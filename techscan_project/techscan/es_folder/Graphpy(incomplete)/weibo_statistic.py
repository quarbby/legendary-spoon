...
with open("Weibo_ML.json") as f: 
    content = json.load(f)


df = pd.DataFrame.from_dict(content, orient='columns')
df_new = df.groupby(['user_name']).sum().reset_index()
print(df_new)
...
for j in range (len(rng)):
	...
	x , y = count_range_in_list(df_new['user_name'],df_new['favorite_count'], j*50, (j+1)*50)
	AuthorList.append(x)
	Totalcount.append(y)
# print (AuthorList)
# print(Totalcount)
...
layout = go.Layout(barmode='stack',
    ...
    title = go.layout.Title(
            text = 'weibo: favorite counts for usernames')
    # violinmode = "group"
	)
data = [trace1,trace2]
fig = go.Figure(data = data, layout=layout)

plot(fig, filename='Weibo upvote statistic.html')
