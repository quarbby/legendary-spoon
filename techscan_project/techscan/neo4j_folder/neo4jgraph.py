...
def related_field(keyword):
  with neo4jdriver.session() as session:
    relatedField = session.run("MATCH (f:Field)<-[:PART_OF_FIELD]-(f1:Field:SubGraphCS) WHERE f.fieldName =~ '(?i).*{}.*' RETURN DISTINCT f1.fieldName,f1.fieldPaperNum ORDER BY f1.fieldPaperNum DESC LIMIT 10".format(keyword))
...
def search_field(Keyword):
	result = []
	with neo4jdriver.session() as session:
		output = session.run('MATCH f=(f1:Field:SubGraphCS)<-[r1:PART_OF_FIELD]-(q:Field:SubGraphCS) WHERE f1.fieldName =~ "(?i).*{}.*" OPTIONAL MATCH e=(q)<-[r2:PART_OF_FIELD]-(w:Field:SubGraphCS) RETURN f,e LIMIT 50'.format(Keyword))
